...
try {
    ...
    try {
        $departments = $db->query("SELECT DISTINCT department_name FROM personnel WHERE department_name <> '' ORDER BY department_name ASC")
    ...
    try {
        $positions = $db->query("SELECT DISTINCT position_name FROM personnel WHERE position_name <> '' ORDER BY position_name ASC")
            ->fetchAll(PDO::FETCH_COLUMN);
    } catch (Exception $e) {
        $positions = array();
    }
    ...
    ajax_json_response(array(
        'success' => false,
        'message' => 'Unable to load personnel data',
    ), 500);
}
